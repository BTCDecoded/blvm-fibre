//! FIBRE: Fast Internet Bitcoin Relay Engine — loadable **`blvm-fibre`** transport.
//!
                ...
                match Self::recv_chunk_internal(&socket).await {
                    Ok((peer_addr, chunk)) => {
                        // Update connection state
                        {
                            ...
                            conn.last_seen = Instant::now();
                            // Note: all chunks for one block share the same `sequence`; dedupe must
                            // not drop later chunks (see `encode_block`). Assembly dedupes by index.
                        }
...
impl FibreRelay {
    ...
    /// Remove a FIBRE peer (e.g. P2P disconnect).
    pub fn unregister_fibre_peer(&mut self, peer_id: &str) -> bool {
        let removed = self.fibre_peers.remove(peer_id).is_some();
        if removed {
            debug!("Unregistered FIBRE peer {}", peer_id);
        }
        removed
    }
    ...
    pub fn encode_block(&mut self, block: Block) -> Result<EncodedBlock, FibreError> {
        ...
        let shard_size = crate::wire::DEFAULT_SHARD_SIZE;
